...
export default function AddRoutineModal({ visible, close, add }: { visible: boolean, close?: Function, add?: Function }) {
  ...
  function ExerciseComponent(props: {
    ...
    return (
      <View style={{ justifyContent: 'space-between', paddingHorizontal: 6, flexDirection: 'row', width: '100%', alignItems: 'center' }}>
        <Text style={{ fontSize: 16, fontWeight: '500' }}>{props.exercise.title}</Text>
        <TouchableOpacity
          onPress={() => remove(props.exercise)}
        >
          <View style={{ padding: 4 }}>
            <MaterialCommunityIcons name="close" size={18} color="#ff8787"></MaterialCommunityIcons>
          </View>
        </TouchableOpacity>
      </View>
    );
